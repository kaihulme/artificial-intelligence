...
%
%% a: movie(american_beauty, Y) .
%% b: movie(M, 2000) .
%% c: movie(M, Y) , Y<2000 .
%% d: movie(M, Y) , Y>1990 .
%% e: actor(M1, A, _) , actor(M2, A, _), M1 \= M2 .
%% f: director(M, D) , actress(M, scarlett_johansson, _) .
%% g: actor(_, A, _), director(_, A) .
%% h: ( actor(_, A, _) ; actress(_, A, _) ) , director(_, A) .
%% i: actor(M, john_goodman, _) , actor(M, jeff_bridges, _) .
%
...
%
%% a:
released_after(M, Y) :- movie(M, X) , X > Y .
%
%% b:
released_before(M, Y) :- movie(M, X) , X < Y .
%
%% c:
